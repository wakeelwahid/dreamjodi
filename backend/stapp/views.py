...
# Forgot password API (no OTP, match username or mobile)
    ...
    if request.method == 'POST':
        try:
            ...
            if not username or not mobile:
                return JsonResponse({'error': 'Both username and mobile number are required.'}, status=400)
            ...
            try:
                ...
                return JsonResponse({'error': 'User not found with provided username and mobile number.'}, status=404)
...
def declare_result(request):
    try:
        ...
        # Diamond King: Remove special result window restriction, treat like other games
            ...
            if game_name in commission_games:
                if bet.user.referred_by:
                    try:
                        ...
                        commission_amount = bet.amount * Decimal('0.05')
...
@api_view(['GET'])
@permission_classes([IsAuthenticated, IsActiveUser])
def user_declare_result_history(request):
    try:
        two_months_ago = timezone.now() - timedelta(days=60)
        # Get all results in last 2 months, newest first
        results = DeclaredResult.objects.filter(declared_at__gte=two_months_ago).order_by('-declared_at')
        # Only keep latest result per (date, game)
        seen = set()
        data = []
        for result in results:
            key = (result.declared_at.strftime('%Y-%m-%d'), result.game_name)
            if key not in seen:
                data.append({
                    "id": result.id,
                    "gameName": result.game_name,
...
def admin_bet_records(request):
    ...
    # Always treat DIAMOND KING as single-session, like other games
    if isinstance(timings, list) and game_upper != "DIAMOND KING":
        today = now.date()
        current_idx = None
    ...
    # Single session per day (including DIAMOND KING)
    else:
        today = now.date()
        open_time = datetime.strptime(timings["open"], "%H:%M").time()
        close_time = datetime.strptime(timings["close"], "%H:%M").time()
        ...
        # For DIAMOND KING, also return current IST for frontend consistency
        if game_upper == "DIAMOND KING":
            return Response({game_upper: {"sessions": sessions_data}, "current_ist": now.strftime("%Y-%m-%dT%H:%M:%S")})
        else:
...
def admin_undo_reset_user(request, user_id):
    ...
    try:
        with transaction.atomic():
            ...
            # Restore bets and keep old_id -> new_id mapping
            Bet.objects.filter(user=user).delete()
            bet_id_map = {}
            for bet in data.get("bets", []):
                old_bet_id = bet.pop('id', None)
                bet['user_id'] = user.id
                bet_obj = Bet.objects.create(**bet)
                if old_bet_id is not None:
                    bet_id_map[old_bet_id] = bet_obj.id

            # Restore referral commissions (remap bet_id if present)
            ReferralCommission.objects.filter(referrer=user).delete()
            ReferralCommission.objects.filter(referred_user=user).delete()
            for rc in data.get("ref_comm_referrer", []):
                ...
                if rc.get('bet_id'):
                    old_bet_id = rc['bet_id']
                    rc['bet_id'] = bet_id_map.get(old_bet_id)
...
from .serializers import AdminPhotoSerializer
...
def admin_delete_photo(request, photo_id):
    try:
        ...
        photo = AdminPhoto.objects.get(id=photo_id)
    except AdminPhoto.DoesNotExist:
...
@api_view(['GET'])
@permission_classes([IsAdminUser])
def admin_get_photos(request):
    photos = AdminPhoto.objects.all()
    serializer = AdminPhotoSerializer(photos, many=True)
    return Response(serializer.data)


# User-facing GET API for photos (only for authenticated users)
@api_view(['GET'])
@permission_classes([IsAuthenticated])
